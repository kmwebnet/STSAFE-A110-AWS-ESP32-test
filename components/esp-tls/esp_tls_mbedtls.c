...
int st_mbedtls_pk_init(StSafeA_Handle_t *pStSafeA, mbedtls_pk_context * pkey, const uint16_t slotid);
static esp_err_t esp_set_stsafe_pki_context(esp_tls_t *tls, const void *pki);
...
#ifdef CONFIG_ESP_TLS_USE_SECURE_ELEMENT
    StSafeA_Reset(&stsafea_handle, STSAFEA_MAC_NONE);
#endif
#ifdef CONFIG_ESP_TLS_USE_DS_PERIPHERAL
    esp_ds_release_ds_lock();
#endif
...
esp_err_t set_server_config(esp_tls_cfg_server_t *cfg, esp_tls_t *tls)
{
    ...
    if (cfg->use_secure_element) {
        ...
        ret = esp_set_stsafe_pki_context(tls, (void*) &pki);
        if (ret != ESP_OK) {
            return ret;
        }
#else
...
#ifdef CONFIG_ESP_TLS_USE_SECURE_ELEMENT
static esp_err_t esp_init_stsafe(uint8_t i2c_addr)
{
    StatusCode = StSafeA_Init(&stsafea_handle, a_rx_tx_stsafea_data);
    if(StatusCode != STSAFEA_OK) {
        ESP_LOGE(TAG, "Failed to initialize stsafe device, returned -0x%04X", -StatusCode);
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
static esp_err_t esp_set_stsafe_pki_context(esp_tls_t *tls, const void *pki)
{
    int ret = 0;
    esp_err_t esp_ret = ESP_FAIL;
    ESP_LOGI(TAG, "Initialize the stsafe interface...");

#if defined(STSAFE_A110)
    esp_ret = esp_init_stsafe(CONFIG_ST_I2C_ADDRESS);
    if (esp_ret != ESP_OK) {
  ...
  uint16_t certlen = 0;
    ...
    ret = mbedtls_x509_crt_parse(&tls->clientcert, (const unsigned char*)cert, (size_t)certlen);
    if (ret != 0) {
        ESP_LOGE(TAG, "Failed to parse cert from device, return 0x%04X", ret);
        mbedtls_print_error_msg(ret);
        return ESP_ERR_ESP_TLS_SE_FAILED;
    ...
    ret = st_mbedtls_pk_init(&stsafea_handle, &tls->clientkey, 0);
    if (ret != 0) {
...
{
    int ret = 0;
    uint8_t public_key[STSAFEA_SIGN_ECDSA_SHA256_LENGTH + 10];
    ...
    uint16_t certlen = 0;
    uint8_t *end_buf;
    end_buf = public_key + STSAFEA_SIGN_ECDSA_SHA256_LENGTH + 1;
    uint8_t *public_key_without_header;
    public_key_without_header = public_key + 1;
    ...
    if (!ret)
    {
        StatusCode = ST_RetrieveCert(pStSafeA, 0, 0, cert, &certlen);
    }

    if (!ret)
    {
        ret = mbedtls_x509_crt_parse(&crt, (const unsigned char*)cert, (size_t)certlen);
    }

    if (!ret)
    {
        ret = mbedtls_pk_write_pubkey(&end_buf, (unsigned char* )&public_key, &crt.pk);
    }

    if (!StatusCode)
    {
        ecp = mbedtls_pk_ec(*pkey);
        ret = mbedtls_ecp_group_load(&ecp->private_grp, MBEDTLS_ECP_DP_SECP256R1);
    }

    if (!ret)
    {
        ret = mbedtls_mpi_read_binary(&(ecp->private_Q.private_X), public_key_without_header, STSAFEA_SIGN_ECDSA_SHA256_LENGTH / 2);
    }

    if (!ret)
    {
        ret = mbedtls_mpi_read_binary(&(ecp->private_Q.private_Y), &public_key_without_header[STSAFEA_SIGN_ECDSA_SHA256_LENGTH / 2], STSAFEA_SIGN_ECDSA_SHA256_LENGTH / 2);
    }
    if (!ret)
    {
        ret = mbedtls_mpi_read_binary(&(ecp->private_Q.private_Z), &temp, 1);
    }
    if (!ret)
    {
        ret = mbedtls_mpi_lset(&ecp->private_d, slotid);
    }
                       ...
                       int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    ...
    if (STSAFEA_XYRS_ECDSA_SHA256_LENGTH > blen || !d || !d->private_p)
    {
    ...
    if (!ret)
    {
        StatusCode = StSafeA_GenerateSignature(&stsafea_handle,*(uint8_t*)d->private_p, buf, STSAFEA_SHA_256,
    ...
    if (!StatusCode)
    {
        ret = mbedtls_mpi_read_binary(r, OutR.Data, (size_t)OutR.Length);
    }

    if (!ret)
    {
        ret = mbedtls_mpi_read_binary(s, OutS.Data, (size_t)OutS.Length);
    }
